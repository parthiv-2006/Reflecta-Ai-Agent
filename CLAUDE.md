...
- **CI / PR integration wraps the loop — it never changes it**: `reflecta ci` = `run_loop` + `ci.submit()`. `ci.py` takes the finished `RunReport` and opens a PR; it does NOT run the loop or touch any gate. Three modules, one concern each: `git_ops.py` (subprocess git: branch/stage/commit/push, all raising `GitError`), `forge.py` (GitHub REST over the already-core `httpx`; `PullRequestHost` protocol + `GitHubHost`, token from `GITHUB_TOKEN`/`GH_TOKEN`, **injectable `httpx.Client` so tests use `MockTransport`** — never the network), and `ci.py` (orchestration + `build_pr_body`). Safety invariants are load-bearing: `ci.submit` stages **only** `tests/_reflecta/` (never `git add .` — hard rule #1), returns `no_tests` (no branch/PR) when `tests_kept==0`, branches off the **current HEAD sha** (not a branch name that may not exist on a detached CI checkout), restores the original branch in a `finally`, and force-with-leases its own `reflecta/auto-tests` branch. Idempotency: after pushing, `host.find_open_pr(head)` → if one is open, report `updated` instead of opening a duplicate. `--dry-run` builds the full `CIPlan` (branch/commit/title/body) and returns **before any git or network** — needs no `GITHUB_TOKEN`. The PR body is built from `RunReport` data only (no per-target mutation score exists there — show the aggregate). `git_ops`/`forge` are injectable into `ci.submit(git=…, host=…)` so the whole flow is unit-tested without git or network; real-git plumbing is covered against a local bare remote.
- **reflecta.toml is run/ci defaults, NOT secrets**: `config.py` stays env/secrets only; `settings.py` (`load_settings`) reads `reflecta.toml` (`[tool.reflecta]` table or top-level, flat keys, `tomllib`). Precedence is **explicit CLI flag > reflecta.toml > built-in default**, implemented with `settings.resolve(cli_value, key, cfg, default)` where toml-overridable CLI options use `Optional[...] = None` sentinels (so "flag not given" is distinguishable). Unknown keys are ignored (a typo can't crash a run); an unparseable file raises `SettingsError`. **Both `run` and `ci`** resolve *every* loop param through `resolve()` — even ones with no CLI flag (e.g. `max_repairs`, `stall_k`) — so reflecta.toml can set them. For this to work, the toml-overridable `run` options are `Optional[...] = None` (matching `ci`); a non-None typer default would make "flag not given" indistinguishable from the default and silently shadow the file (the bug that let `mutation = true` be honored by `ci` but ignored by `run`). `run` resolves `python`/`skip_entrypoints`/`attempt_risky` *before* the `--dry-run` branch so the no-quota triage preview honors the file too.
...
│       ├── cli.py               # typer commands: run / ci / triage / clean / report
│       ├── loop.py              # main orchestration loop
│       ├── ci.py                # reflecta ci: wrap run_loop, open a PR of kept tests
