...
- [ ] **0.5. Shared data model.**
  ...
  - Files: `src/coverloop/models.py`, `tests/test_models.py` (light: instantiate each dataclass, assert field names and defaults).
  - Verify: `from coverloop.models import CoverageTarget, GeneratedTest, RepairAttempt, RunReport` works from any module. No task after this point defines its own version of these types.
...
- [ ] **2.5. LLM provider wrapper.**
  ...
  - Files: `src/coverloop/llm/provider.py`, `tests/test_provider.py`.
...
- [ ] **3. Gemini generation with a real prompt.**
  - Does: `generate_test(target, source, existing_tests)` builds a prompt with the full source module, existing tests, and the exact missed lines, calls Gemini Flash via the provider wrapper, extracts a clean pytest file, writes it to the `_coverloop` path. The per-module counter is determined at write time by scanning existing `_coverloop/test_coverloop_<module>_*.py` files — no manifest needed, no collisions across runs.
...
- [ ] **7. Coverage-delta gate.**
  ...
  - Files: `src/coverloop/gates.py`, `src/coverloop/loop.py` (stub), `tests/test_gates_delta.py`.
...
- [ ] **8a. Happy-path loop.**
  ...
  - Files: `src/coverloop/loop.py`, `tests/test_loop_happy.py`.
...
- [ ] **8b. Repair loop and budget.**
  ...
  - Files: `src/coverloop/loop.py` (extended), `src/coverloop/budget.py`, `tests/test_loop_budget.py`.
...
- [ ] **9. CLI and run report.**
  ...
  - Verify:
    - `coverloop run --path examples/sample_project --max-iters 1` completes end to end from a clean state in under 2 minutes.
    - `coverloop clean` sub-verify: set up a fixture with both `tests/_coverloop/test_coverloop_calc_0.py` (generated) and `tests/test_calc.py` (human-written). Run `coverloop clean`. Confirm only the `_coverloop` file is removed; `tests/test_calc.py` is untouched. This is a hard rule — a bug here deletes human tests.
    - `coverloop report --last` reprints the JSON report from the previous run without re-running.
